...
public:
    ...
    static T readParam(ros::NodeHandle &n, std::string name)
    {
        ...
        if (!n.getParam(name, ans)) {
            ROS_FATAL_STREAM("Failed to load " << name);
            exit(1);
        }
        return ans;
    }
    ...
    template <typename T>
    static T readParam(ros::NodeHandle &n, std::string name, const T& alternative)
    {
        T ans;
        n.param<T>(name, ans, alternative);
        return ans;
    }

    static Eigen::Vector3d readVector3d(ros::NodeHandle &n, const std::string& name)
    {
    ...
    static geometry_msgs::Point32 getPointMsg(const Eigen::Vector3d& p)
    {
...
struct LidarConfig {
    ...
    LidarConfig(ros::NodeHandle& nh, const std::string& prefix) {
