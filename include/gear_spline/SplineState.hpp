...
// ROS2 message includes - only for ROS2 builds
// For ROS1/standalone builds, these are excluded
#if defined(USE_ROS2_MSGS) && !defined(STANDALONE_TEST)
...
class SplineState
{
  ...
  public:
    ...
    // === Milestone 2: Dynamic control point access ===

    // Get current active control point count
    int getActiveN() const { return active_N_; }

    // Get maximum control point count
    static constexpr int getMaxN() { return MAX_N; }

    // Whether in uniform mode
    bool isUniform() const { return is_uniform_; }

    // Set uniform mode
    void setUniformMode(bool uniform) { is_uniform_ = uniform; }

    // Get dynamic-sized RCPs (active_N * 6 dimensions)
    Eigen::VectorXd getRCPsDynamic() const {
        Eigen::VectorXd RCPs(active_N_ * 6);
        int start_idx = num_knot - active_N_;
        for (int i = 0; i < active_N_; i++) {
            RCPs.segment<3>(i * 6) = t_knots[start_idx + i];
            RCPs.segment<3>(i * 6 + 3) = ort_delta[start_idx + i];
        }
        return RCPs;
    }

    // Update dynamic-sized RCPs
    void updateRCPsDynamic(const Eigen::VectorXd& cp) {
        int start_idx = num_knot - active_N_;
        for (int i = 0; i < active_N_; i++) {
            t_knots[start_idx + i] = cp.segment<3>(i * 6);
            ort_delta[start_idx + i] = cp.segment<3>(i * 6 + 3);
            Eigen::Quaterniond q_del;
            Quater::exp(ort_delta[start_idx + i], q_del);
            if (start_idx + i > 0) {
                q_knots[start_idx + i] = q_knots[start_idx + i - 1] * q_del;
            } else if (start_idx + i == 0) {
                q_knots[start_idx + i] = q_idle[2] * q_del;
            }
        }
    }

    // Grow window: allow one more control point in estimation
    void growWindow() {
        if (active_N_ < MAX_N && active_N_ < num_knot) {
            active_N_++;
        }
    }

    // Set active control point count
    void setActiveN(int n) {
        active_N_ = std::max(4, std::min(n, std::min(MAX_N, (int)num_knot)));
    }

    // Get dt history for non-uniform mode
    const std::vector<double>& getDtHistory() const { return dt_history_; }

    // Add dt to history (for non-uniform mode)
    void addDtToHistory(double dt) {
        dt_history_.push_back(dt);
        // Keep history size in sync with active window
        while ((int)dt_history_.size() > active_N_) {
            dt_history_.erase(dt_history_.begin());
        }
    }

    // Get the RCP start index (first control point in active window)
    int getRCPStartIndex() const {
        return std::max(0, (int)num_knot - active_N_);
    }
  ...
  private:
    ...
    bool is_uniform_ = true;              // Whether uniform time intervals
    int active_N_ = 4;                    // Current active control point count (4-15)
    std::vector<double> dt_history_;      // Non-uniform time interval history [dt_0, dt_1, ...]
    static constexpr int MAX_N = 15;      // Maximum control point count
